...
function fig_1_plot(inpv,d1,d2,levin,tit)
    f2 = Figure(;
    ...
    ax = GeoAxis(f2[1,1];
        ...
        bb = contourf!(ax, d1, d2, inpv,
             levels = levin,
             #levels = range(273, 303, length = 21), # rh
             colormap = :bam,
...
function fig_1_anom(inpv,d1,d2,levin,tit)
    f2 = Figure(;
        figure_padding=(5,5,10,10),
        backgroundcolor=:white,
        size=(700,300),
    ...
    ax = GeoAxis(f2[1,1];
        ...
        ylabel="latitude",
        xlabel="longitude",
        limits=(-180,180,-30,30),
        title=tit,
        )
        bb = contourf!(ax, d1, d2, inpv,
             levels = levin,
             #levels = range(-0.15, 0.15, length = 21), # rh
             #colormap = :bam,
             colormap = :vik,
             #colormap = :batlow,
             extendlow = :auto, extendhigh = :auto
...
# do we want to use +/- 30 or +/- 40 to define the tropical regions?
lat1 = 50
lat2 = 130
...
sst_tr_nino          = Array{Union{Missing, Float64}, 1}(undef, ensoEnd)
sst_tr_nina          = Array{Union{Missing, Float64}, 1}(undef, ensoEnd)
#sst_mn = mean(sst_var, dims=3)
#sst_tr_mn = mean.(skipmissing(sst_var[:,lat1:lat2,:]));

#sst_tr_mean[1] = mean(skipmissing(sst_var[:,lat1:lat2,1]))
#test = mean(skipmissing(sst_var[:,lat1:lat2,1]))
...
#ensoEnd = 48
for i in 1:ensoEnd
    sst_tr_nino[i] = mean(skipmissing(sst_nino[:,lat1:lat2,i]))
    sst_tr_nina[i] = mean(skipmissing(sst_nina[:,lat1:lat2,i]))
end
...
print("size of MPI Nina mean is: ",size(mpi_nina_mn))

mpi_comp = mpi_nino_mn .- mpi_nina_mn
...
# Degrees/34 years right?  Then to convert to degrees per century 'agrid1' could be multiplied by 
# 2.94
